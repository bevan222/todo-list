...
connectToDB();
function filterTasksWithSortOption(sortOption, results) {
    switch (sortOption) {
        case 1:
...
function filterTasksWithCreator(creator, results) {
    if (creator === '') {
        return results;
    }
    return results.filter((result) => result.creator.toLowerCase().includes(creator.toLowerCase()));
}
function filterTasksWithTime(startDate, endDate, results) {
...
//Task API
app.post('/task/getFilterTask', (req, res) => {
...
app.get('/task/getAllTasks', (req, res) => {
    pool.query('SELECT tasks.id as id, tasks.task_name as "taskName", tasks.creator_id as "creatorId", tasks.created_at as "createTime", tasks.duedate as "dueDate", tasks.complete as complete, tasks.description as description, users.username as creator, task_history.mod_time as modTime FROM tasks LEFT JOIN users ON tasks.creator_id = users.id LEFT JOIN task_history ON task_history.task_id = tasks.id ORDER BY tasks.id;', (error, results) => {
        if (error) {
            res.status(400).send({ message: error });
            throw error;
...
app.put('/task/modTaskComplete', (req, res) => {
...
app.delete('/task/deleteTask', (req, res) => {
    const { id } = req.body;
    if (id === undefined) {
        res.status(400).send(`no taskId provided`);
        return;
...
//Comment API
app.post('/comment/getTaskComment', (req, res) => {
    const { taskId } = req.body;
    pool.query('SELECT comments.id as id, comments.message as message, comments.created_at as "createTime", belong_task_id as "belongTaskId", users.username as creator FROM comments LEFT JOIN users ON comments.creator_id = users.id WHERE comments.belong_task_id = ($1) ORDER BY comments.created_at;', [taskId], (error, results) => {
        if (error) {
            res.status(400).send({ message: error });
            throw error;
        }
...
app.post('/comment/createComment', (req, res) => {
...
app.put('/comment/modComment', (req, res) => {
...
app.delete('/comment/deleteComment', (req, res) => {
